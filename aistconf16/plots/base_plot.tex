...
\definecolor{LightRed}{rgb}{0.65, 0.45, 0.45}
\definecolor{DarkGreen}{rgb}{0.35, 0.55, 0.35}
\definecolor{DarkYellow}{rgb}{0.7, 0.7, 0.5}
\definecolor{Blue}{rgb}{0.65, 0.65, 0.85}
...
\newcommand*{\Normalization}[2][XXX]{
	\bitvector{#1}{#2}{DarkYellow}{DarkYellow}{}{1.0}
}
...
\newenvironment{wave}[5][\footnotesize\bf{time} $\:$]{
	...
	\draw[dotted] (t_cur) +(0,-0.1) node[above] {\footnotesize 0\%} -- ++(0,#5-#2);
	\clki
	...
	\foreach \t in {#4,...,#4}{
		\draw[dotted] (t_cur) +(0,-0.1) node[above right] {\footnotesize \t$\,$sec.} -- ++(0,#5-#2);
		\clki
	}
}{\end{tikzpicture}}
