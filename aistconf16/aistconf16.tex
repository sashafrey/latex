...
\section{Offline algorithm (\kw{OfflineARTM})}
...
\kw{Offline ARTM} is described by Alg. \ref{alg:OfflineARTM} and \ref{alg:ProcessDocument}.
...
This step is not strictly necessary for \kw{Offline ARTM},
...
Fig. \ref{fig:gantt:OfflineARTM} shows a Gantt chart of the \kw{Offline ARTM} execution.
...
The \kw{Online ARTM} (Alg. \ref{alg:OnlineARTM}) generalize
the Online variational Bayes algorithm,
suggested in \cite{hoffman10online} for the LDA model.
Online algorithm improves the convergence rate of the \kw{Offline ARTM}
...
The color indicate which version of the $\Phi$ matrix was used to process each batch
(orange for even $i$, yellow for odd $i$).
Blue box correspond to the time spend in merging $n_{wt}$ with $\hat n_{wt}$.
Green box is, as before, the time spent to normalize $n_{wt}$ counters into the new $\Phi$ matrix.
...
The \kw{Async ARTM} algorithm is the old non-deterministic online algorithm from \kw{BigARTM v0.6}, described in \cite{vfardi15aist}.
Fig. \ref{fig:gantt:AsyncARTM}, \ref{fig:gantt:AsyncARTM:slow} illustrates run-time behavior of the \kw{Async ARTM} algorithm.

TBD: write this chapter.

% Hack: omit Algorithm word to save one line.
%\section{Deterministic Asynchronous Online Algorithm (\kw{DetAsync ARTM})}
\section{Deterministic Asynchronous Online (\kw{DetAsync ARTM})}
...
We would like to thank Prof. K.~V. Vorontsov for constant attention to our research
...
\newblock {\em Machine Learning, Special Issue on Data Analysis and Intelligent Optimization}, volume 101(1), pages 303--323, 2015.
