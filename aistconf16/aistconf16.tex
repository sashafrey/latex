...
that is known to be faster comparing to other packages for topic modelling such as Vowpal Wabbit or Gensim.
%which currently is the most performant open-source library for topic modeling.  
%K.V.: which is faster comparing to other open-source packages for topic modelling such as Vowpal Wabbit or Gensim.
...
In our experiments the new implementation spends less time to achieve the same perplexity, has better CPU utilization and lower memory usage.
...
To learn $\Phi$ and $\Theta$ from $(n_{td})$ ARTM maximize
the log-likelihood $\cL(\Phi, \Theta)$, regularized via an additional penalty term $R(\Phi, \Theta)$:
\begin{align}
    \cL (\Phi,\Theta) =
    \sum_{d\in D}\sum_{w\in W} n_{dw} \ln \sum_{t\in T} \phi_{wt} \theta_{td}, \\
    \cL (\Phi, \Theta) + R(\Phi, \Theta) 
    \;\to\; \max_{\Phi,\Theta}.
\end{align}
...
Learning of $\Phi$ and $\Theta$ can be done via Offline EM-Algorithm
