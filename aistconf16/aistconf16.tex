...
The \kw{Async} algorithm is the old non-deterministic online algorithm from \kw{BigARTM v0.6}, described in \cite{vfardi15aist}.
Fig. \ref{fig:gantt:AsyncARTM}, \ref{fig:gantt:AsyncARTM:slow} illustrates run-time behavior of the \kw{Async} algorithm.
...
We presented a deterministic asynchronous algorithm for learning
additively regularized topic models (ARTM).
The algorithm is flexible and covers a reach class of multi-objective topic models,
much wider than Latent Dirichlet Allocation (LDA).
We provided an efficient implementation of the algorithm in BigARTM open-source library,
and our solution converges an order of magnitude faster than
