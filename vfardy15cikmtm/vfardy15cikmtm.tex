...
\author{
    ...
    \alignauthor
        Peter Romov\\
        \affaddr{Yandex}\\
        \email{\footnotesize\sf romovpa@yandex-team.ru}
    \alignauthor
        Marina Suvorova\\
        \affaddr{Moscow State University}\\
...
\category{I.2.7}{Natural Language Processing}{Language \mbox{models}}
\category{G.1.6}{Optimization}{Constrained optimization}
%\category{G.1.3}{Numerical Linear Algebra}{Sparse, structured, and very large systems (direct and iterative methods)}

\terms{Theory, Algorithms, Experimentation}
...
\SetAlgoSkip{}
\begin{algorithm2e}[t]
\caption{Online EM for multimodal ARTM} %\mbox{regularized} topic modeling
\label{alg:Online}
\BlankLine
...
\begin{figure}[t]
\begin{centering}
\includegraphics[height=38mm]{diagramm_artm_core.eps}
...
\label{th:multimodal}
    ...
    of the problem~\eqref{eq:multimodal},~\eqref{eq:multimodal:norm}
    then for any regular topic~$t$ %for a~modality~$m$ 
