...
A careful selection of the programming tools is quite important for any software project.
This is especially true for BigARTM as its code is written in C++,
a language that on its own offers less functionality comparing to Python, .NET Framework or Java.
To mitigate this we use
various parts of the Boost C++ Libraries,
Google Protocol Buffers for data serialization,
Google C++ Testing Framework,
Google Logging library for C++,
Google Commandline flags module for C++,
ZeroMQ library for network communication,
RPCZ library as an RPC implementation for Google Protocol Buffers.
...
and Boost Libraries 1.46.1 or newer. All the other third-parties are included in the BigARTM repository.

To organize the project we use several free online services,
including \url{https://github.com/} --- to hose the source code,
\url{https://readthedocs.org/} --- to host online documentation,
\url{http://travis-ci.org} --- to run automated continuous integration builds.
...
Each run in our experiment performs one pass over the Wikipedia corpus and produce a model with $|T|=100$ topics.
...
The update rule in online algorithm used $\rho = 1 \slash \sqrt{b + \tau_0}$, where $b$ is the number of batches processed so far,
and $\tau_0$ is an a constant offset parameter, introduced in \cite{hoffman10online} (in our experiment $\tau_0 = 64$).
...
Table \ref{tab:top10words} shows top 10 words for some of the topics.
...
\begin{table}[t!]
    ...
    \begin{tabular}{|l|l||l|l||l|l|}	
    	...
    	%student(0.01) & наука(0.01) & debut(0.01) & профиль(0.01) \\
    	%program(0.01) & бизнес(0.01) & contract(0.01) & сезон(0.01) \\
    	%studies(0.01) & международный(0.01) & signed(0.01) & команда(0.01) \\
    	%universities(0.01) & организация(0.01) & loan(0.01) & чемпионат(0.01) \\
    	%academic(0.01) & научный(0.01) & friendly(0.01) & дебютировать(0.01) \\
    	%learning(0.01) & факультет(0.01) & championship(0.01) &  лига(0.01) \\
    	%programs(0.01) & исследование(0.01) & premier(0.01) & футбол(0.01) \\
    	%information(0.01) & технология(0.01) & playing(0.01) & сыграть(0.01) \\
    	%association(0.01) & технический(0.01) & squad(0.01) & молодежный(0.01) \\
    	%sciences(0.01)& заведение(0.01) & profile(0.01) & контракт(0.01) \\
    	\hline
    	...
    	%recordings(0.01) & премия(0.01) & visual(0.01) & приложение(0.01) \\
    	%recording(0.01) & исполнять(0.01) & client(0.01) & studio(0.01)\\
    	%singing(0.01) & симфонический(0.01) & mail(0.01) & enterprise(0.01) \\
    	%performances(0.01) & фестиваль(0.01) & software(0.01) & services(0.01) \\
    	%festival(0.01) & музыкант(0.01) & apache(0.01) & управление(0.01) \\
    	%sang(0.01) & конкурс(0.01) & nt(0.01) & пользователь(0.01) \\
    	%recorded(0.01) & запись(0.01) & edition(0.01) & сервис(0.01) \\
    	%mozart(0.01) & сопрано(0.01) & management(0.00) & служба(0.01) \\
    	%classical(0.01) & петь(0.01) & net(0.00) & кокс(0.01)\\
    	%soprano(0.01) & моцарт(0.01) & xp(0.00) клиент(0.01)& \\
      	\hline
	\end{tabular}
    \vskip 2ex
	\caption{
        Top10 words of two-language topic model,
        based on all Russain and English Wikipedia articles with mutual interlanguage links.}
