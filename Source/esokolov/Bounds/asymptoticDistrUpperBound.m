function [bound, worstBound] = asymptoticDistrUpperBound(algs, sources, L, ell)
    nAlgs = algs.Count;
    ...
    % вероятности выбора для каждого алгоритма
    probs = zeros(nAlgs, 1);
    for i = 1:nAlgs
        prob = Inf;
        
        u = algs.Data(i).upperNeighsCnt;
        for j = 1:size(sourcesVects, 1)
            a = sum(algs.Data(i).errVect < sourcesVects(j, :)');
            b = sum(algs.Data(i).errVect > sourcesVects(j, :)');
            currProb = 0;
            for t = 0:min(a, b)
                currProb = currProb + ((CnkCalc(b, t) * ...
                    CnkCalc(L - u - b, ell - u - t)) / CnkCalc(L, ell));
            end
            prob = min(prob, currProb);
        end
        
        probs(i) = prob;
    end
    
    [bound, worstBound] = asymptoticDistrUpperBound_inner(errMatrix, probs, ell, L);
end

function [ expectation , worst ] = asymptoticDistrUpperBound_inner( A, probabilities, l, L )
...
for k = 0:m_max
    ...
    if ~isempty(ind)
        for j = 1:length(ind)
            value = value + probabilities(ind(j));
        end
    end
