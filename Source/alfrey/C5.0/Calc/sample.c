...
/*  ----  */
{
    ...
    void		ShowRules(FILE*, int);

    /*  Process options  */

    while ( (o = ProcessOption(Argc, Argv, "f+xrRs")) )
    {
	switch (o)
	{
	...
	/* Write information about predicted target classes */

	if ( ! (OutputFile = GetFile(".out", "w")) ) Error(NOFILE, Fn, "");
	...
	fprintf(OutputFile, "%-15s %*s   [Predicted]%s\n\n",
    ...
    {
		if (!SimpleOutput) {
			fprintf(OutputFile, "Case\t\tGiven\t\tPredicted%s\n %s\t\tClass\t\tClass\n\n",
				( RULESUSED ? "\t\t    Rules" : "" ),
				( LabelAtt ? "ID" : "No" ));
		}
	...
	if (!SimpleOutput) {
		if ( LabelAtt )
		{
			fprintf(OutputFile, "%-15.15s ", (String) (IgnoredVals + SVal(Case,LabelAtt)));
		}
		else
		{
			fprintf(OutputFile, "%4d\t\t", ++CaseNo);
		}
	}
	...
	if ( XRefForm )
	{
	    fprintf(OutputFile, "%*s", -MaxClassLen, ClassName[Class(Case)]);
	    CurrentPosition = 16 + MaxClassLen;

	    if ( Class(Case) != Predict )
	    {
		fprintf(OutputFile, "   [%s]", ClassName[Predict]);
	...
	else
	{
		if (!SimpleOutput) 
		{
			fprintf(OutputFile, "%-15.15s %-15.15s [%.2f]",
				ClassName[Class(Case)], ClassName[Predict], Confidence);
			CurrentPosition = 54;
		} 
		else
		{
			fprintf(OutputFile, "%-15.15s", ClassName[Predict]);
			CurrentPosition = 4;
		}			
	}

	if ( RULESUSED ) ShowRules(OutputFile, StartList - CurrentPosition);
...
void ShowRules(FILE* file, int Spaces)
/*   ---------  */
{
	...
	ForEach(a, 1, RulesUsed[p])
	{
	    ...
	    if ( TRIALS > 1 ) fprintf(file, "%d/", Trial);

	    fprintf(file, "%d ", RulesUsed[First]);
