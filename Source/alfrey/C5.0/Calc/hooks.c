...
/*   -----------  */
{
	...
	case BrDiscr:  /* test of discrete attribute */
	    ...
	    {
		FollowAllBranches(Case, T, Fraction, Prob);
	    }
...
/*   -----  */
{
    ...
    if ( ErrNo == NOMEM )
    {
	MessageBoxC("Cannot allocate sufficient memory", "Fatal Error", MB_ICONERROR | MB_OK);
	Goodbye(1);
    }
    else
    if ( ErrNo == MODELFILE )
	...
	if ( ! ErrMsgs )
	{
	    sprintf(Msg, "File %s is incompatible with .names file\n(%s `%s')",
			 Fn, S1, S2);
	    MessageBoxC(Msg, "Cannot Load Classifier", MB_ICONERROR | MB_OK);
	}
	ErrMsgs++;
	return;
    }
    ...
    if ( ErrMsgs <= 10 )
    {
	MessageBoxC(Buffer, ( WarningOnly ? "Warning" : "Error" ), MB_OK);
    }
#else
    fprintf(Of, Buffer);
#endif
    ...
    {
#if defined WIN32 && ! defined _CONSOLE
	MessageBoxC(T_ErrorLimit, "Too many errors!", MB_OK);
#else
...
// This allocates memory. Caller must free resulting variable.
wchar_t* WIDESTR(const char* str) 
{
	int len = MultiByteToWideChar(CP_ACP, 0, str, strlen(str) + 1, 0, 0);
	wchar_t* buf = (wchar_t*)malloc(len * sizeof(wchar_t));
	len = MultiByteToWideChar(CP_ACP, 0, str, strlen(str) + 1, buf, len);
	return buf;
}

void MessageBoxC(const char* message, const char* caption, UINT type) {
	wchar_t* captionW = WIDESTR(caption);
	wchar_t* messageW = WIDESTR(message);
	MessageBox(NULL, messageW, captionW, type);
	free(captionW);
	free(messageW);
}