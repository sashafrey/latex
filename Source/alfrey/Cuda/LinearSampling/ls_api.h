...
// Performs monte-carlo cross-validation over the set of error vectors.
...
// Returns: actual count of neighbors found, or negative value if error happens during evaluation.
// Supports onlineCV mode. In this mode random walk calculates Monte-Carlo estimate of ERM along with the walk.
// Information about overfitting, produced by online MCC, can be exported at several "checkpoints".
// Checkpoints correspond to the number of algorithms found.
// We recommend to pass (W_output == NULL) and (isSource == NULL) to enable longer walks in OnlineMCC mode.
LINEARSAMPLING_API int runRandomWalking(
    ...
    int                 randomSeed,
    int                 onlineCV_nIters,        //                                          - number of MCC sample splits to perform in OnlineCV mode
    int                 onlineCV_nCheckpoints,  //                                          - number of checkpoints to export results of OnlineCV mode.
    const int           *onlineCV_checkpoints,  // onlineCV_nCheckpoints * 1                - the list of checkpoints for OnlineCV mode
    int                 onlineCV_nTrainItems,   //                                          - number of items in train set for OnlineCV mode
    ...
    int                 *onlineCV_trainEC,      // onlineCV_nIters * onlineCV_nCheckpoints  - for each sample split and checkpoint of OnlineCV mode reports train errors count of the best ERM alg.
    int                 *onlineCV_testEC        // onlineCV_nIters * onlineCV_nCheckpoints  - for each sample split and checkpoint of OnlineCV mode reports test errors count of the best ERM alg.
    );         
